...
document.getElementById("add-comm-btn").addEventListener("click", addComment);
...
function addComment() {
    ...
    if(comment_content.length > 30) {
        errors.innerHTML += '<article class="error-msg">&#9888; Comment must contain maximum 30 characters.</article>';
        valid = false;
    }
    if(valid) {
        var comment = {
        ...
        http.onreadystatechange = function() {
            if(http.readyState == 4 && http.status == 200) {
                var data = JSON.parse(http.responseText);
                if(data.success != '') {
                    success.innerHTML = '<article class="success-msg">New comment added.</article>';
    ...
    comment_list.forEach(function(comment){
        user_list.forEach(function(user){
            if(comment.user_id == user.id) {
                comments_container.innerHTML += '<p class="comment-author">';
                if(user.image == null) {                    
